...
* 缓存穿透：在某些情况下，大量对于同一个数据的访问，经过了缓存屏障直接访问数据库，造成数据库连接资源占用激增等情况，甚至造成数据库异常。
  * 解决方法：1）缓存中预先缓存“key不存在orkey对应的value不存在”； 2）“布隆过滤器算法”：在数据库查询前预先过滤调某些不存在的结果【没见过的数据就一定是没见过】。
...
* 在存储系统中，客户端对数据的读写行为都是可预期、符合一定规则的。
  * 可预期和符合规则（非“数据一致、准确”）：因为不同类型的一致性模型，数据对一致性遵从的程度和规则都不同。
  * **存储系统实际存储的数据可以在某些时候不遵从一致性，只需要保证客户端能读到一致的数据即可。**

### 一致性模型

* 强一致性Strong Consistency：任意时间下，读操作总能取得最近一次写操作写入的数据。
* 弱一致性Weak Consistency：任意时间下，**读操作并不能保证可以取得最近一次写操作写入的数据。**
* 最终一致性Eventual Consistency：介于强弱一致性之间，写操作之后经过一定时间之后，读操作就一定能读到这次写操作写入的数据。

## 设计数据持久层

* 持久化框架选择、持久层代码设计、存储技术选型等。
* 数据库范式Normal Form：
  * 第一范式 ：**每个属性值都是不可再分的**（数据库中所有数据属性都简化到不可再分）
  * 第二范式：**去除局部依赖---表中的属性完全依赖于全部主键而非部分主键。**
  * 第三范式：**去除非主属性的传递依赖**（第二范式的基础上，<u>非主属性必须直接依赖于主键</u>，而不能传递依赖于主键。）

# 关于搜索引擎

* 组成部分：
  * Crawling：爬虫定时访问网站，若数据发生变化，爬虫将可访问网页下载下来。【常规引擎发现网页的方式，需要网页主动向引擎告知有这么个页面，否则爬虫也无法发现。】
  * Indexing：将爬取的结果使用“**倒排索引**”的方法对网站索引进行建立，方便各类网页进行归类存放。
  * Serving Results：拆解用户的搜索条件，根据**多种因素**来决定返回哪些url给用户以及这些url的展示顺序。【百度广告多的原因之一】
* PageRank：浏览器返回结果质量的最大差别。
  * 早期：依赖对于搜索关键字和目标页的匹配程度排序。【无法解决关键字堆砌的垃圾网页排序较高的问题】
  * PageRank算法本质：利用网页之间的关联关系（url）来确定网页影响力权重。
  * 实际搜索过程需要做到两个因素的平衡：Reputation(网页影响力，它并不会因为用户单次搜索关键字不同而改变)；Proximity(接近程度、根据用户搜索的关键词匹配程度来确定返回网页)：
* SEO相关技术：
  * 白帽White Hat：通过正当和高效的技术途径来实现SEO的效果。
  * **黑帽Black Hat**：通过作弊、欺骗等手段实现SEO效果。
    * 关键字堆砌：放置大量的甚至和网页内容无关的关键字，以欺骗搜索引擎增加曝光度。
    * 链接农场Link Farm：将网站链接放到很多本不该进行外链的其他网站页面上（提高网站引用权重、深度增加被搜索到的概率。）
    * Artical spinning：将一些其他网站已有内容拷贝过来，做一些用来欺骗搜索引擎的修改，让搜索引擎以为是一份新的内容。
  * 站内优化：在自己管理的网站内部做优化来实现SEO。
    * **网页->右键->“检查源码”->"keywords"**:该网页的搜索权重关键词。
  * 站外优化：向外提供链接的优化方式。
  * **robots.txt**: 网站根目录下直接能访问到的文本文件，是一个对爬虫的**规约**，**告诉爬虫这个网站哪些内容可以爬，哪些不能爬。**【不是标准也不是规范，只是“约定俗成”】
    * 此文件中设置：**crawl-delay**用于说明每次数据爬取至少间隔多少S。
    * eg：www.baidu.com--- https://www.baidu.com/robots.txt/
  * **网站地图**：通过网站地图清晰直接的告诉搜索引擎，重要的页面有哪些、更新习惯、频率等。
    * 关键词：sitemap
  * 统计分析：在SEO的改动调整后，需要一些方式来跟踪和评估效果。【eg：google analytics 和 百度统计】
    * 网站用户情况统计：访问量、页面停留时间、地区分布、电脑访问/手机访问比例等。
  * 
