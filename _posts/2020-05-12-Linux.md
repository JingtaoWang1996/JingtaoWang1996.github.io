...
基于rsync协议传输，远程主机也需要安装rsync同时启动rsync守护进程。【使用虚拟用户和密码可以解决普通用户权限不足的问题】

* 传输时，若未指定协议，则默认使用ssh协议。指定rsync方式如下：
  * rsync -av /source rsync://ip/path to source file

**[具体配置示例](https://cloud.tencent.com/developer/article/2151454)**
...
​       PS: 底层文件系统，先挂载到 VFS 目录树中的某个子目录（称为挂载点），然后才能访问其中的文件。拿第一类，也就是基于磁盘的文件系统为例，在安装系统时，要先挂载一个根目录（/），在根目录下再把其他文件系统（比如其他的磁盘分区、/proc 文件系统、/sys 文件系统、NFS 等）挂载进来。
