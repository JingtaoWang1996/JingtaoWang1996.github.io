...
* cd iftop-0.17，执行命令配置：./configure --prefix=/usr/local/iftop

**问题记录**

1、报错：configure: error: cannot guess build type; you must specify one

​      原因：国产麒麟平台，无法猜测build 类型，需要指定。

​      解决办法： 

​      1） 指定操作系统：./configure --build=arm-linux

2、报错：can't find pcap.h [执行上述命令后]
