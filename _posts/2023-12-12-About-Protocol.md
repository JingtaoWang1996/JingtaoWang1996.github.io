...
**推流-RTMP协议：**将数据流打包传输到对端。

* RTMP协议基于TCP，需要双方建立TCP连接，在TCP连接的基础上建立RTMP连接【用于双方沟通版本号、时间戳等信息】。

### P2P协议

解决单一服务器接收多个请求时的带宽压力。【参与的人越多，下载越快】

* 想要下载某一大文件时，只需与已经存了文件的peer建立点对点连接，就可以就近下载文件。
* 下载完毕之后，成为peer中的一员，旁边的服务器也可以从这里下载已经有的文件。
...
* .torrent文件组成：announce(tracker URL) & 文件信息。
  ...
  * info区：该种子有几个文件、文件长度、目录结构等。
  * Name区：指定顶层目录名字。
  * 每个段的大小：BitTorrent协议将文件分成很多小段，然后分段下载。
  * 段hash值：将整个种子种，每个段的SHA-1hash值拼在一起。

* 下载时，BT客户端先解析.torrent文件，得到tracker地址，然后连接tracker服务器。

* tracker服务器回应下载者的请求，将其他下载者（包括发布者）的IP提供给下载者，下载者再链接其他下载者，双方建立连接后交换对方没有的数据块。

* 此时不需要其他服务器参与，减轻了服务器负担。

### CDN-content Delivery Network

CDN分发系统的架构：

* 边缘节点：分布在各个地方的数据中心的节点。
* 区域节点：边缘节点之上有区域节点，缓存下更多的东西。
* 中心节点：区域节点之上，所有数据存储的地方。

CDN适合缓存的数据类型：

* 静态页面：内容通过拉取的方式，逐级向上确认直到获得需要的数据。
* 流媒体数据：通过拉取的方式，数据量较大，因此多采取主动推送的方式将热点数据推送到边缘节点，减小压力。
* 动态数据：
  * 边缘计算，数据动态生成。
  * 路径优化，通过CDN网络分发，减少压力。
...
* 网络命名空间namespace：由**ip netns 命令**操作，可创建、删除、查询namespace。
* 机制网络**cgroup**：control group，linux 内核提供的一种可以限制、隔离进程使用的资源机制。可控制的资源如下：
  * CPU、cpuset、memory、blkio(设置限制每个块设备的输入输出)、net_cls（允许linux流量控制程序识别从cgroup中生成的数据包）

**Flannel**

* 通过注册中心，可以对多个docker服务的备份&调度平台进行控制。
* Flannel通过udp实现overlay网络。
* 给不同物理机设置不同的网段。

**Calico网络模型设计思路**

* 不走overlay网络，不引入另外的网络性能损耗，将转发全部用三层网络的路由来实现。
...
* 工作方式：通过隧道技术在公网上仿真一条点到点的专线，**利用一种协议来传输另一种协议的技术**。以ipsec协议为例：

  | 外层IP头 | IPSec头  | 内层IP包 |
  | -------- | -------- | -------- |
  | 承载协议 | 隧道协议 | 乘客协议 |

  * 乘客协议：带传输的数据被封装过程需要满足的协议。
  * 隧道协议：实际期望执行的协议。
  * 承载协议：传输过程需要满足的协议，用于封装期望执行的隧道协议。

* IPsec VPN-基于IP协议的安全隧道协议。
  * 私密性：传输加密之后的密文数据。【对称加密随地】
  * 完整性：通过hash运算，产生类似指纹的数据摘要，保证数据完整性。
  * 真实性：数据由特定的对端发出，通过身份认证保证数据真实性。
  * IPsec vpn的协议簇：
    * AH: authentication Header 只能进行数据摘要，不能实现数据加密。
    * ESP：Encapsulating Security Payload 能够进行数据加密和数据摘要。
  * 建立过程：
    * 建立IKE自己的SA：SA用于维护一个通过身份认证和安全保护的通道，通过**Diffie-Hellman算法计算出一个对称密钥k。**
    * 基于对称密钥K，建立IPsec SA: 双方生成一个随机的对称密钥M,由K加密传输给对方，然后使用M进行双方进行接下来通信的数据。对称密钥M过期后，会重新生成一次，防止被破解。

## 云安全

* 对于公有云上的虚拟机，仅开放需要的端口，其他端口一律关闭，只需要通过安全措施守护好这一个端口即可。
  * ACL：access control list 访问控制集合，又称安全组。
  * iptables配置规则：通过filter进行规则配置。
* 网络的Quality of Service：流量控制确保一台公用wifi上各个设备的网速。控制方式：
  * linux：TC 控制QoS-队列方式。
  * 入口流量无法控制，但出口流量多基于队列控制。
* 隔离GRE、VXLAN：
  * 仅就与VLAN的12位，共4096个隔离策略不够用的情况，解决方式：原来包的基础上**增加一个扩展头**，用于区分用户。
  * GRE：Generic Routing Encapsulatiuon，一种ip-over-ip的隧道技术，将ip包封装在GRE当中，外面加上ip头，到另一端时解封。【此时的隧道是虚拟&点对点的连接。】
    * GRE是一种点对点隧道，若网络数量过多，隧道数量也会指数级增长。
    * GRE不支持组播。【点对点】
    * 很多防火墙和三层网络设备无法解析GRE。
  * VXLAN：区别于GRE，从二层外面就套一个VXLAN的头进行封装。
    * VTEP： 对VXLAN的包进行封装和解封。
    * VXLAN不是点对点的，支持组播方式发送信息。
  * 
