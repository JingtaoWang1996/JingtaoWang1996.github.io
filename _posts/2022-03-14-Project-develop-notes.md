...
* 瀑布模型：从需求明确到开发到测试，上一步完成了才能进行下一步的顺序开发模型。
  * 优点：需求明确、阶段性可控、
  * 缺点：需求更改成本消耗大、时间成本无法估计、无法快速迭代。
* 快速原型模型：解决客户需求不明确和需求多变的问题。
  * 迅速构建一个可运行的软件原型，收集用户反馈、反复修改确定最终需求。
  * 优点：快速修改、对用户反馈和变更能及时做出响应。
  * 缺点：这种原型开发代码的质量往往是最大的代价。
  * 因此，在确认完需求后，原型会被抛弃，重构代码。(Or )

* 大瀑布拆小瀑布：解决瀑布模型周期长、响应困难的问题
  * 增量模型-按模块分批次交付
  * 迭代模型-逐渐优化迭代版本：每个迭代都有可用的版本。
  * 螺旋模型：高风险项目，出现问题及时止损。
* 敏捷开发：
  * 需要客户全程参与，测试相对较少，遇到问题需要随时调整。
  * 与瀑布模型的差异：
    * 瀑布模型需要详尽的需求分析文档，敏捷开发可以简化为用户需求的故事，优先进入开发过程。

## 项目可行性研究

* **如果发现方案不具备可行性，应及时调整方案或停止项目以止损。**
* 主要考虑的三个方面：经济可行性（投入产出比）、技术可行性（技术上是否可行）、社会可行性（设计法律、道德、社会影响等因素）
* 如果可行性研究并不能有一个明确的结果，可以先小范围试点，等验证可行了再大范围投入。

## 需求分析

* 挖掘真实需求：大部分用户提的需求都不是其真实需求，需要透过现象看本质，看**真正需要解决的问题是什么。**
* 提出解决方案：结合真正需要解决的问题+实际使用场景，提出可行的解决方案。
* 筛选和验证方案：对于提出的方案进行筛选和验证，确保在用户可接受成本内能解决问题。
