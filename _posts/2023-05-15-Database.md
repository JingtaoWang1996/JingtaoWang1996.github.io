...
* 一次性选取某几列的值：select colA,colB from table_name;

* 获取当前数据库中所有表名称

```python
def _get_all_table_name(self):
    get_table_name_sql = "select name from sqlite_master where type='table'"
    self.cur.execute(get_table_name_sql)
    temp_tab_name = self.cur.fetchall()
    tab_name = []
    for line in temp_tab_name:
        if not line[0] == "sqlite_sequence":
            tab_name.append(line[0])
    return tab_name
```
