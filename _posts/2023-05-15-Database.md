...
* 添加GPG key：wget -qO - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo apt-key add -
* 更新源：sudo apt update 
* 安装最新版：sudo apt install -y postgresql-16 postgresql-contrib
* 验证安装版本：psql --version
## 配置
* 进入postgreSQL：
  * sudo -u postgres psql
* 创建数据库：CREATE DATABASE leo;
* 创建用户：CREATE USER leo WITH PASSWORD 'cetc@30';
* 授权访问数据库：GRANT ALL PRIVILEGES ON DATABASE leo TO leo;
### 开启远程监听
* 编辑postgresql.conf: sudo vim /etc/postgresql/18/main/postgresql.conf
  * 找到并修改：`listen_addresses = 'localhost,你的服务器IP'`
* 编辑放行规则文件pg_hba.conf: sudo vim /etc/postgresql/18/main/pg_hba.conf
  * 文件末尾添加[开发阶段]：`host    all     all     0.0.0.0/32     md5`
  * 生产阶段:`host    all     all     允许访问的IP地址/32     md5`
* 确保允许用户远程登录：
  * 进入postgreSQL：  sudo -u postgres psql
  * 设置密码【理论上来说上面创建用户设置了这里就不需要了】：ALTER USER netuser WITH PASSWORD 'netpass123';
...
**Q1:创建schema必须在正确的db中，否则就会没有权限创建表**
* 登录数据库leo: sudo -u postgres psql -d leo
* 查看leo下的schema：\dn
  * 如果出了public没有看到别的，则表示之前的schema创建失败
* **在正确的数据库中执行** : CREATE SCHEMA leo;
  * 验证是否成功：\dn【看到leo后表示成功】
  * 给用户权限：GRANT ALL ON SCHEMA leo TO leo;
