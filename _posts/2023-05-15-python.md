...
生成器仅在被使用时调用，不需要一次性生成全部元素，减小内存占用。
...
* 运行pyinstaller：
  ...
  * **如果主脚本是main.py**：pyinstaller --onefile main.py

    * 这将会把所有内容打包成一个单独的二进制文件，保存在dist文件夹中。

  * **如果希望打包其他资源或额外配置，如：配置文件**

    * pyinstaller --onefile --add-data "config.json:." 

      `--onefile`：将所有内容打包成一个独立的可执行文件。

      `--add-data`：将额外的文件（如配置文件、资源文件等）包含在可执行文件中。注意在 Linux 系统中，使用冒号 `:` 分隔源路径和目标路径。

  * 打包完成后，pyinstaller会在该项目的目录下生成一个dist文件夹，里面有可执行的二进制文件。
...
​       PS: 依赖写入requirements.txt 中：pip freeze >requirements.txt

* conda 安装依赖：conda install --file requirements.txt
...
​      多个except 模块都匹配的情况下，只有最上方一个会被执行。
...
通过迭代器的方法每次读取文件的每行，能够减少系统io。[参考](https://blog.csdn.net/m0_61394395/article/details/134379624)
...
局限性：想用好asyncio，必须要相应的库兼容aiohttp。
...
* 需要创建一个TestSort继承类“unittest.TestCase"。
  ...
  * 这个类中不需要通过init来初始化，每个test函数中包含所有测试依赖&输入。
...
说明：上面例子中定义了A 类的三个方法 m1()、m2()、m3()。需要对m1()进行单元测试，但m1()取决于m2()和m3()。若m2()和m3() 的内部比较复杂, 就不能简单地调用 m1() 进行测试，可能需要解决很多依赖项的问题。
...
* 打开项目->在导航中点击一个类、包或者模块。
* 选择 `Diagrams > Show Diagram`
* 类图会在新窗口中展示，可以展开或折叠类的依赖关系
* 右键选中任意一个类：`Content > Show Category` 后增加field 和 method

**继续编辑**

* 全部选中之后：`Export Diagram > Export to File> Plant UML`
* 文件保存后，在保存的地方根据需要筛选和增加类型数据

## 远程调试

* 首先需要注册账号：[edu账号注册专业版](https://blog.csdn.net/fengbao24/article/details/125515542)

### 远程调用配置

* pycharm专业版中设置远程服务器
  * **Tools-->Deployment-->Configuration**
  * 选择左上角+，选择SFTP连接，随意取个名字
  * 设置ssh参数
  * 在**connection**页面，**Root path**填写服务器上代码所在地址
  * 在**mapping **界面，将**Local path（本地工程地址）** 和 **Deployment path（服务器代码地址，填写“/”）**
* 完成上面一步的配置并“ok”后，选中更新方式： **Tools-->Deployment-->Automatic Upload**
* 设置代码上传及更新方式：**Tools-->Deployment-->Options ** --“Upload changed files automatically to the default server“  ---”选择ctrl+S“

### pycharm中配置解释器

* file->settings->python interpreter ->add interpreter
* SSH interpreter 
  * 输入ssh服务器的ip地址，host和username
  * 登录后点击右边方框，选择服务器的interpreter
  * **Running code on the remote server** 右侧的方框，设置Local path（本地地址）& Remote Path （服务器代码地址）
  * 选中remote上的python解释器后，再点击编辑

* pycharm中设置对应的远程调用的intepreters 
  * 选择ssh：
  * ssh configuration：选择之前已经配好的配置
  * python interpreter path：选择远程服务器的解释器
  * 不勾选 Execute code using this interpreter with root priviledges via sudo
* 设置终端运行的python版本:File ->Settings ->Tools ->SSH terminal
  * 在SSH configuration 中，选中配置好的Remote python

### 本地拉取和上传代码

* 左侧先选中本地工程文件后
* Tools--> Deployment --> Download from(从服务器下载代码)// Upload to（上传代码）
...
**设置软连接：直接通过python3命令调用新安装的python3**

* sudo ln -s /usr/local/bin/python3.x /usr/local/bin/python3

## 问题记录

* 现象：Centos7 安装python3之后运行报错
  ...
  * 永久解决方案：
    ...
    * vim ~/.bashrc
    * 在文件最后加上上面的两行命令保存后执行：source ~/.bashrc 使修改永久生效。
...
* 环境迁移 --- 本机克隆
  ...
  2） 激活新环境：conda activate 新环境

### 环境打包及迁移到其他服务器

* 安装conda-forge & conda-pack 打包工具：conda install –c conda-forge conda-pack
* 激活要打包的环境：conda activate 要打包的环境
* 打包环境：conda pack -n 要打包的环境 -o myenv.tar.gz

**内网服务器上部署**

* 查看conda环境：conda env list
* 将xxx.tar.gz 拷贝到conda安装目录中:多为/root/anaconda3/env/下面
* 在conda安装路径下创建环境文件夹：mkdir **mal_domain_detection**
* 将 xxx.tar.gz 解压到上述创建的文件夹中：tar -xzf myenv.tar.gz -C **mal_domain_detection**
* 激活测试：conda activate **mal_domain_detection**

## 换源

* 查看当前软件源：conda config --set show_channel_urls yes
...
* 验证是否成功更改软件源，执行下述代码后输出源：conda config --show channels 
...
* **centos7编译安装conda步骤**
  * 安装依赖：
    ...
    * sudo yum install libstdc++-devel libffi-devel openssl-devel
  * 下载conda源码：
    * wget https://github.com/conda/conda/archive/master.zip 
    * unzip master.zip
  * 进入conda源码目录执行命令编译安装：
    * 
  * wget https://repo.anaconda.com/archive/Anaconda3-2024.02-1-Linux-x86_64.sh
  * 执行以下命令后，按照步骤安装conda：bash Anaconda3-2024.02-1-Linux-x86_64.sh
  * 初始化 anaconda：
...
* conda uninstall xxx提示：missing xxx 
  * 解决方案：最初安装时可能是pip install 而非conda install, 通过channel可以确定。

# miniconda

## 安装配置-ubuntu

[ref](https://blog.csdn.net/weixin_44491423/article/details/127023092)
...
* 执行上述命令并按照叙述流程进行执行，直到出现“Thank you for installing Miniconda3！”
...
* 打开文件并进行一系列操作，也不能直接用assert**【因为文件可能不存在且打开过程还可能有其他异常，】**因此需要用try catch 处理
